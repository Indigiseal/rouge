---
type: month
project: Evershift
status: example
month_number: 8
name: Brassfair
theme: Cheerful wrongness
location: The Brassfair Grounds
boss: The Ringmaster
melee_count: 3
ranged_count: 2
archetypes: skirmisher,bruiser,swarm,artillery,artillery
tags:
  - evershift
  - month
  - example
---

# Brassfair

The month of false lanterns, cheap wonders, and deals that smell of brass. In this turn danger often smiles and offers to sell you something wonderful.

## Theme

Cheerful wrongness — lanterns, deals, and machines that should not smile

## Location

### The Brassfair Grounds

**The Brassfair Grounds** is the signature place of this month.

Roster encountered here: [[Faircutter]], [[Clockwork Barker]], [[Brass Imp]], [[Prize Rifleman]], [[Fortune Lobber]].  
Boss: [[The Ringmaster]].

## Cast

Required roster: **3 melee**, **2 ranged**, **1 boss**. Archetype multipliers: [[Enemy Power]] / `docs/BALANCE.md`.

### Melee (3)

| Enemy | Role | Archetype | Status | Note |
|---|---|---|---|---|
| [[Faircutter]] | melee | `skirmisher` | proposed | Masked carnival attendant with a prize-blade. |
| [[Clockwork Barker]] | melee | `bruiser` | proposed | Noisy construct that closes distance. |
| [[Brass Imp]] | melee | `swarm` | proposed | Small scrap-demon chaff. |

### Ranged (2)

| Enemy | Role | Archetype | Status | Note |
|---|---|---|---|---|
| [[Prize Rifleman]] | ranged | `artillery` | proposed | Carnival gunner. |
| [[Fortune Lobber]] | ranged | `artillery` | proposed | Throws cursed trinket bombs. |

### Boss (1)

| Boss | Status | Note |
|---|---|---|
| [[The Ringmaster]] | proposed | Brass-and-cloth showman of the fair. |

> [!note] Links
> Enemy and boss names link to [[Enemies Index]] / [[Bosses Index]] pages.

## Features

- Carnival logic: junk, fortunes, illegal machines.
- Brass, cogs, and smiling merchants who should not exist.
- Natural home for the music-box lane's tone.

## Events

- [[08 Brassfair/Events/Something Wicked|Something Wicked]] — A wrong carnival under lanterns. An old woman sells cursed-cheap trinkets and will not easily let go.
- [[08 Brassfair/Events/The Brass Wizard|The Brass Wizard]] — A coin-slot fortune machine behind carnival curtains. Fortunes, card swaps, jams, or a machine that wakes with human eyes.
- [[08 Brassfair/Events/The Broken Music Box|The Broken Music Box]] — A cracked brass music box under rubble. It may explode, follow you, or start a whole companion lane once its missing cog is found.

## Soft fits

- [[03 Tollroad/Events/Goblin Engineer|Goblin Engineer]] _(soft)_ — A goblin offers to repair the Latchbox — properly for coin or card parts, or poorly for free. Success unlocks the box's drawers.

## Opening text (draft)

> The month of **Brassfair** has begun.  
> The Waystar did not lose its way. The world did.

## Related

- [[Enemies Index]]
- [[Bosses Index]]
- [[Enemy Power]]

- [[Months Index]]
- [[The Calendar]]
- [[_Shared/Music Box Chain|Music Box Chain]]
- [[_Shared/Any-Month Events|Any-Month Events]]


